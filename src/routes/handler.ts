...
export class FileRouteHandler {
  ...
  constructor(config: FileHandlerConfig) {
    ...
    this.serveUrlBuilder = config.serveUrlBuilder || ((key) => "/api/files/serve/" + encodeURIComponent(key));
  }

  private getContext(contextInput?: string): string {
    return contextInput &&
      this.storageManager.hasContext(contextInput)
      ? contextInput
      : "general";
  }
  ...
  async handleDownload(key: string, name?: string, contextInput?: string) {
    ...
    if (this.storageManager.supportsPresignedUrls(context)) {
      const downloadUrl = await this.storageManager.generatePresignedDownloadUrl({
        key,
        context,
  ...
  async handlePresigned(
    input: { fileName: string; contentType: string; fileSize: number; context?: string },
  ...
  async handleBatchPresigned(
    input: { files: Array<{ fileName: string; contentType: string; fileSize: number }>; type?: string },
    metadata?: Record<string, any>
  ) {
  ...
  async handleMultipart(
      ...
      if (!this.storageManager.supportsMultipartUpload(context)) {
          throw new Error("Provider for " + context + " doesn't support multipart upload");
      }

      switch (action) {
          case 'initiate': {
              const { fileName, contentType, fileSize, metadata: clientMetadata } = data;
  ...
  async handleUpload(
      ...
      return uploadResults.length === 1 ? uploadResults[0] : { files: uploadResults };
  }
